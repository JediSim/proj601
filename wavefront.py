...
class WavefrontOBJ:
    ...
    def boundary_vertices( self ):
        """permet d’extraire les bords dans un maillage (i.e. les arêtes incidentes à une seule face). La méthode ci-dessous retourne les sommets sur le bord, sous forme d’une liste []."""
        bdry_v= set()
        darts = {}
    ...
    def ordered_boundary( self ):
        """La méthode ci-dessous reconstruit la liste ordonnée des sommets sur le bord (si il y a un seul bord)."""
        edges    = self.boundary_edges()
        if len(edges)==0:
            return []
        d = {}
